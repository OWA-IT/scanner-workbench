...
- `SCANNER_IMAGE`: Docker image tag used by `docker-compose.yml`
- `SCANNER_PORT`: Host port mapped to the container's port 8000
- `SCANNER_DATA_DIR`: Host directory used to persist SQLite data
...
- The Docker setup uses a single app container with Gunicorn and a persistent SQLite bind mount. That is the intended initial deployment model for small standalone client installs.
